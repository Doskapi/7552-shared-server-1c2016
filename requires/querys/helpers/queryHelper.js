...
//AGREGA METADATA CON COUNT
QueryHelper.addMetadataWithCount = function(data,count){
  data.metadata = {
    version: cStatus.VERSION,
    count: count
  };
};

//AGREGA METADATA
QueryHelper.addMetadata = function(data){
  data.metadata = {
    version: cStatus.VERSION
  };
};
...
QueryHelper.validatePersonalUserData = function(user){
  return (user.name !== undefined &&
    ...
    user.photo_profile !== undefined && (user.sex == 'male' || user.sex == 'female' || user.sex =='men' || user.sex=='women'));
  };
